...
self.addEventListener('push', (event: PushEvent) => {
  ...
  if (isJSON(event.data.text())) {
    ...
    // Push notification
    if (push?.ecmMessageId || push?.fcmMessageId) {
      if (push.data?.type === 'notification') {
        const notificationOption: INotificationOptions = {
          ...push.notification,
          data: push.data,
            ...
            .then((list) => {
              if (list.length > 0) {
                list.forEach((client: Client) => {
                  ...
                  if (isExzlyMessaging) {
                    notificationOption.data.ecmMessageId = push.ecmMessageId;
                  }

                  if (isFirebaseMessaging) {
                    notificationOption.data.fcmMessageId = push.fcmMessageId;
                  }
                  ...
                  client.postMessage({
                    ...
                    notification: {
                      ...push.notification,
                      data: notificationOption.data,
                    },
                  });

                  // Send feedback
                  webPushFeedback('received', notificationOption.data);
                });
              } else {
                ...
                // Send feedback
                webPushFeedback('displayed', notificationOption.data);
              }
            }),
        );
      }
    ...
    clients
      ...
      .then(() => {
        if (clickedNotification.data) {
          // Send feedback
          webPushFeedback('clicked', clickedNotification.data);
        }
      }),
  );
});
...
self.addEventListener('notificationclose', (event: NotificationEvent) => {
  ...
  if (closedNotification.data) {
    // Send feedback
    webPushFeedback('closed', closedNotification.data);
  }
});
